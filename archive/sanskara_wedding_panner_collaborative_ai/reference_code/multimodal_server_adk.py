...
# Function tool managing wedding tasks
def todo_tasks_wedding_tool(request:dict) -> dict:
    """Function tool to get the wedding related tasks.
        Args:
            request (dict): The request containing the wedding tasks info/status/user preferences etc.
        Returns:
    ...
    with open(file_name, 'r+') as f:
        tasks = json.load(f)
        # Update the tasks with the new request
...
class MultimodalADKServer(BaseWebSocketServer):
    ...
    def __init__(self, host="0.0.0.0", port=8765):
        ...
        self.agent = Agent(
            ...
            tools=[todo_tasks_wedding_tool,google_search],
        )
    ...
    async def process_audio(self, websocket, client_id):
        ...
        async with asyncio.TaskGroup() as tg:
            ...
            # Task to handle incoming text messages (if any)
