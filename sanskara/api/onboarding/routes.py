...
async def submit_onboarding_data(submission: OnboardingSubmission | SecondPartnerSubmission, background_tasks: BackgroundTasks):
    ...
    if not user_id:
        logging.error(f"Onboarding failed: User with email {user_email} not found in the users table. User must sign up first.")
    ...
    if existing_wedding_id and wedding_id and existing_wedding_id != wedding_id:
        logging.warning(f"Onboarding conflict: User {user_email} is already associated with wedding {existing_wedding_id}, but attempted to link to {wedding_id}.")
        ...
        if submission.current_user_onboarding_details.email == submission.partner_onboarding_details.email:
            logging.warning(f"Onboarding submission failed: Current user ({user_email}) and partner have the same email address.")
...
async def get_partner_details(email: str):
    ...
    if not wedding_data:
        logging.info(f"Partner details request for {email}: No wedding found where this email is an expected partner.")
