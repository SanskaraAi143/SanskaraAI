...
async def add_guest(wedding_id: str, guest_name: str, side: str, contact_info: str) -> str:
    ...
    """
    with logger.contextualize(wedding_id=wedding_id, guest_name=guest_name, side=side):
        logger.debug(f"Attempting to add guest: {guest_name} for wedding {wedding_id}")
        try:
            sql_query = """
            ...
            if result.get("status") == "success" and result.get("data"):
                ...
                logger.info(f"Guest '{guest_name}' added successfully with ID: {guest_id}")
                return f"Guest '{guest_name}' added successfully with ID: {guest_id}"
            else:
                error_message = result.get("error", "Unknown error")
                logger.error(f"Error adding guest: {guest_name}. Error: {error_message}")
                return f"Error adding guest: {error_message}"
        except Exception as e:
            logger.error(f"Error adding guest: {guest_name}. Error: {e}", exc_info=True)
...
async def update_guest_rsvp(guest_id: str, rsvp_status: str) -> str:
    ...
    Returns:
        A message indicating the success or failure of the operation.
    """
    with logger.contextualize(guest_id=guest_id, rsvp_status=rsvp_status):
        logger.debug(f"Attempting to update RSVP status for guest_id: {guest_id} to {rsvp_status}")
        try:
            sql_query = """
            UPDATE guest_list
            SET status = :status
            ...
            if result.get("status") == "success" and result.get("data"):
                logger.info(f"Guest '{guest_id}' RSVP status updated to '{rsvp_status}' successfully.")
                ...
                if "0 rows" in error_message: # Specific error message for no rows updated
                    logger.warning(f"Guest with ID '{guest_id}' not found for RSVP update.")
                    return f"Guest with ID '{guest_id}' not found."
                logger.error(f"Error updating guest RSVP status for guest_id: {guest_id}. Error: {error_message}")
                return f"Error updating guest RSVP status: {error_message}"
        except Exception as e:
            logger.error(f"Error updating guest RSVP status for guest_id: {guest_id}. Error: {e}", exc_info=True)
...
async def send_email(recipient_email: str, subject: str, body: str) -> str:
    ...
    """
    with logger.contextualize(recipient_email=recipient_email, subject=subject):
        ...
        try:
            ...
            logger.error(f"Exception when sending email: {e}", exc_info=True)
...
async def send_whatsapp_message(phone_number: str, template_name: str, params: dict, body_text: Optional[str] = None) -> str:
    ...
    """
    with logger.contextualize(phone_number=phone_number, template=template_name):
        logger.debug(f"Preparing WhatsApp message to {phone_number}")

        if not twilio_client:
            logger.warning("Twilio client not initialized (missing SID / token).")
            return "Twilio credentials not configured."
        if not (TWILIO_WHATSAPP_NUMBER or TWILIO_MESSAGING_SERVICE_SID):
            logger.warning("No TWILIO_WHATSAPP_NUMBER or TWILIO_MESSAGING_SERVICE_SID configured.")
        ...
        try:
            if use_content_api:
                ...
                logger.debug(f"Sending via Content API mode (content_sid={template_name})")
                # Remove None keys
                ...
                logger.debug(f"Sending plain body WhatsApp message (len={len(body_text)})")
...
async def delete_guest(guest_identifier: str) -> str:
    ...
    """
    with logger.contextualize(guest_identifier=guest_identifier):
        logger.debug(f"Attempting to delete guest with identifier: {guest_identifier}")
        try:
            sql_query = """
            UPDATE guest_list
            SET status = 'deleted'
            ...
            if result.get("status") == "success" and result.get("data"):
                ...
                logger.info(f"Guest '{deleted_guest_id}' marked as deleted successfully.")
                return f"Guest '{deleted_guest_id}' marked as deleted successfully."
            else:
                error_message = result.get("error", "Unknown error")
                if "0 rows" in error_message:
                    logger.warning(f"Guest with identifier '{guest_identifier}' not found for deletion.")
                    return f"Guest with identifier '{guest_identifier}' not found."
                logger.error(f"Error deleting guest with identifier: {guest_identifier}. Error: {error_message}")
                return f"Error deleting guest: {error_message}"
        except Exception as e:
            logger.error(f"Error deleting guest with identifier: {guest_identifier}. Error: {e}", exc_info=True)
...
if __name__ == "__main__":
    ...
    async def run_real_tests():
        ...
        # except Exception as e:
        #     logger.error(f"Error during add_guest test: {e}", exc_info=True)
        ...
        #     except Exception as e:
        #         logger.error(f"Error during update_guest_rsvp success test: {e}", exc_info=True)
        # else:
        ...
        # except Exception as e:
        #     logger.error(f"Error during update_guest_rsvp not found test: {e}", exc_info=True)
        ...
        # except Exception as e:
        #     logger.error(f"Error during send_email test: {e}", exc_info=True)
        ...
        try:
            ...
            logger.error(f"Error during send_whatsapp_message test: {e}", exc_info=True)
        ...
        # except Exception as e:
        #     logger.error(f"Error during send_whatsapp_message no credentials test: {e}", exc_info=True)
        # finally:
    ...
    #         except Exception as e:
    #             logger.error(f"Error during delete_guest success test: {e}", exc_info=True)
    #     else:
    ...
    #     except Exception as e:
    #         logger.error(f"Error during delete_guest not found test: {e}", exc_info=True)
    ...
    try:
        ...
        logger.error(f"An error occurred during guest and communication agent testing: {e}", exc_info=True)
    finally:
        logger.info("Guest and Communication Agent tests script finished.")