...
async def add_item_to_mood_board(wedding_id: str, image_url: str, note: Optional[str] = None, category: str = 'Decorations') -> Dict[str, Any]:
    ...
    """
    logging.info(f"wedding_id={wedding_id}, image_url={image_url}, category={category}")
    logging.debug(f"Attempting to add item to mood board for wedding_id: {wedding_id}, category: {category}")
    ...
    if mood_boards_result.get("status") == "success" and mood_boards_result.get("data"):
        ...
        logging.debug(f"Found existing mood board with ID: {mood_board_id}")
    
    if not mood_board_id:
        logging.info(f"No mood board found for wedding_id: {wedding_id}. Creating a new one.")
        ...
        if create_mb_result.get("status") == "success" and create_mb_result.get("data"):
            ...
            logging.info(f"Successfully created new mood board with ID: {mood_board_id}")
        else:
            logging.error(f"Failed to create mood board for wedding_id: {wedding_id}. Error: {create_mb_result.get('error')}", exc_info=True)
    ...
    if result.get("status") == "success":
        ...
        logging.info(f"Successfully added item to mood board. Item ID: {item_id}")
        return {"status": "success", "item_id": item_id}
    else:
        logging.error(f"Failed to add item to mood board. Error: {result.get('error')}", exc_info=True)
...
async def generate_and_add_to_mood_board(
    ...
    """
    logging.info(f"wedding_id={wedding_id}, prompt={prompt[:50] + '...' if len(prompt) > 50 else prompt}, category={category}")
    logging.info(f"Generating image and adding to mood board for wedding {wedding_id}")
    ...
    try:
        ...
        if generation_result.get("status") != "success":
            logging.error(f"Image generation failed: {generation_result.get('error_message')}")
            return {
                "status": "error",
        ...
        if not image_url:
            logging.error("No image URL available from generation result")
            return {
                "status": "error",
        ...
        if mood_board_result.get("status") != "success":
            logging.error(f"Failed to add generated image to mood board: {mood_board_result.get('message')}")
            return {
        ...
        logging.info(f"Successfully generated image and added to mood board. Item ID: {mood_board_result.get('item_id')}")
        ...
        logging.error(f"Error in generate_and_add_to_mood_board: {e}", exc_info=True)
        return {
            "status": "error",
...
async def upload_and_add_to_mood_board(
    ...
    """
    logging.info(f"wedding_id={wedding_id}, filename={filename}, category={category}, image_size={len(image_data)}")
    logging.info(f"Uploading image and adding to mood board for wedding {wedding_id}")
    ...
    try:
        ...
        if not supabase_url:
            logging.error("Failed to upload image to Supabase storage")
            return {
                "status": "error",
        ...
        if mood_board_result.get("status") != "success":
            logging.error(f"Failed to add uploaded image to mood board: {mood_board_result.get('message')}")
            return {
                "status": "partial_success",
        ...
        logging.info(f"Successfully uploaded image and added to mood board. Item ID: {mood_board_result.get('item_id')}")
        ...
        logging.error(f"Error in upload_and_add_to_mood_board: {e}", exc_info=True)
        return {
            "status": "error",
...
async def upload_and_add_to_mood_board_b64(
    ...
    """
    logging.info(f"tool_name='upload_and_add_to_mood_board_b64', wedding_id={wedding_id}, filename={filename}, category={category}, mime_type={mime_type},")
    try:
    ...
    try:
        ...
        logging.error(f"Error in upload_and_add_to_mood_board_b64: {e}")
...
async def get_mood_board_items(wedding_id: str, mood_board_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    logging.info(f"wedding_id={wedding_id}, mood_board_id={mood_board_id}")
    logging.debug(f"Retrieving mood board items for wedding {wedding_id}")
    ...
    try:
        ...
        if items_result.get("status") != "success":
            logging.error(f"Failed to retrieve mood board items: {items_result.get('error')}")
            return {
                "status": "error",
        ...
        logging.info(f"Retrieved {len(items)} mood board items")
        ...
        logging.error(f"Error retrieving mood board items: {e}", exc_info=True)
        return {
            "status": "error",
