...
async def get_tasks(wedding_id: str, filters: Dict[str, Any] = None) -> List[Dict[str, Any]]:
    ...
    """
    with logger.contextualize(tool_name="get_tasks", wedding_id=wedding_id, filters=filters):
        logger.debug(f"Entering get_tasks tool for wedding_id: {wedding_id} with filters: {filters}.")
        try:
            ...
            if result.get("status") == "success" and result.get("data"):
                tasks = result["data"]
                logger.info(f"Successfully retrieved {len(tasks)} tasks for wedding {wedding_id}.")
                return tasks
            logger.info(f"No tasks found for wedding {wedding_id}.")
            return []
        except Exception as e:
            logger.error(f"Error in get_tasks for wedding {wedding_id}: {e}", exc_info=True)
            return []
...
async def update_task_status(task_id: str, new_status: str) -> Dict[str, Any]:
    ...
    """
    with logger.contextualize(tool_name="update_task_status", task_id=task_id, new_status=new_status):
        logger.debug(f"Entering update_task_status tool for task_id: {task_id} with new_status: {new_status}.")
        try:
            ...
            logger.error(f"Failed to update task {task_id} status to {new_status}. Result: {result}")
            return {"status": "failure", "message": result.get("message", "Failed to update task status.")}
        except Exception as e:
            logger.error(f"Error in update_task_status for task {task_id}: {e}", exc_info=True)
...
async def submit_task_feedback(task_id: str, user_id: str, related_entity_id: Optional[str], comment: str) -> Dict[str, Any]:
    ...
    """
    with logger.contextualize(tool_name="submit_task_feedback", task_id=task_id, user_id=user_id, related_entity_id=related_entity_id):
        logger.debug(f"Entering submit_task_feedback tool for task_id: {task_id}, user_id: {user_id}, comment: {comment[:50]}...")
        try:
            ...
            if result.get("status") == "success" and result.get("data"):
                ...
                logger.info(f"Successfully submitted feedback for task {task_id}. Feedback ID: {feedback_id}")
                return {"feedback_id": feedback_id}
            logger.error(f"Failed to submit feedback for task {task_id}. Result: {result}")
            return {"status": "failure", "message": result.get("message", "Failed to submit feedback.")}
        except Exception as e:
            logger.error(f"Error in submit_task_feedback for task {task_id}: {e}", exc_info=True)
...
async def approve_task_final_choice(task_id: str, user_id: str) -> Dict[str, Any]:
    ...
    """
    with logger.contextualize(tool_name="approve_task_final_choice", task_id=task_id, user_id=user_id):
        logger.debug(f"Entering approve_task_final_choice tool for task_id: {task_id}, user_id: {user_id}.")
        try:
            ...
            if result.get("status") == "success":
                ...
                logger.info(f"Successfully recorded final approval for task {task_id} by user {user_id}.")
                return {"status": "success", "is_fully_approved": is_fully_approved}
            logger.error(f"Failed to record approval for task {task_id}. Result: {result}")
            return {"status": "failure", "message": result.get("message", "Failed to approve task.")}
        except Exception as e:
            logger.error(f"Error in approve_task_final_choice for task {task_id}: {e}", exc_info=True)
...
async def create_timeline_event(wedding_id: str, event_name: str, event_date_time: str, location: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    with logger.contextualize(tool_name="create_timeline_event", wedding_id=wedding_id, event_name=event_name):
        logger.debug(f"Entering create_timeline_event tool for wedding_id: {wedding_id}, event_name: {event_name}.")
        try:
            ...
            if result.get("status") == "success" and result.get("data"):
                ...
                logger.info(f"Successfully created timeline event '{event_name}' for wedding {wedding_id}. Event ID: {event_id}")
                return {"event_id": event_id}
            logger.error(f"Failed to create timeline event '{event_name}' for wedding {wedding_id}. Result: {result}")
            return {"status": "failure", "message": result.get("message", "Failed to create timeline event.")}
        except Exception as e:
            logger.error(f"Error in create_timeline_event for wedding {wedding_id}: {e}", exc_info=True)
