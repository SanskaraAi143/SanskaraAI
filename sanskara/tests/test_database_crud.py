...
async def test_weddings_crud():
    ...
    try:
        # 1. Insert Test
        logger.info(f"Attempting to insert wedding: {wedding_name}")
        insert_sql = """
        ...
        if insert_result.get("error") or not insert_result.get("data"):
            logger.error(f"Insert wedding failed: {insert_result.get('error', 'No data returned')}")
            test_passed = False
        ...
        if inserted_wedding.get("wedding_id") != wedding_id or inserted_wedding.get("wedding_name") != wedding_name:
            logger.error(f"Insert wedding verification failed. Expected ID {wedding_id}, got {inserted_wedding.get('wedding_id')}. Expected name {wedding_name}, got {inserted_wedding.get('wedding_name')}")
            test_passed = False
        else:
            logger.info(f"Successfully inserted wedding: {inserted_wedding}")

        # 2. Read Test
        if test_passed:
            logger.info(f"Attempting to read wedding with ID: {wedding_id}")
            ...
            if read_result.get("error") or not read_result.get("data"):
                logger.error(f"Read wedding failed: {read_result.get('error', 'No data returned')}")
                test_passed = False
            else:
                read_wedding = read_result["data"][0]
                if read_wedding.get("wedding_name") != wedding_name:
                    logger.error(f"Read wedding verification failed. Expected name {wedding_name}, got {read_wedding.get('wedding_name')}")
                    test_passed = False
                else:
                    logger.info(f"Successfully read wedding: {read_wedding}")

        # 3. Update Test
        if test_passed:
            logger.info(f"Attempting to update wedding with ID: {wedding_id}")
            update_sql = """
                UPDATE weddings
            ...
            if update_result.get("error") or not update_result.get("data"):
                logger.error(f"Update wedding failed: {update_result.get('error', 'No data returned')}")
                test_passed = False
            else:
                ...
                if updated_wedding.get("wedding_name") != updated_name or updated_wedding.get("status") != updated_status:
                    logger.error(f"Update wedding verification failed. Expected name {updated_name}, status {updated_status}. Got {updated_wedding.get('wedding_name')}, {updated_wedding.get('status')}")
                    test_passed = False
                else:
                    logger.info(f"Successfully updated wedding: {updated_wedding}")

    finally:
        # Cleanup: Delete the created wedding
        logger.info(f"Cleanup: Attempting to delete wedding {wedding_id}")
        ...
        if delete_result.get("error") or not delete_result.get("data"):
            logger.error(f"Cleanup failed: Could not delete wedding {wedding_id}: {delete_result.get('error', 'No data returned')}")
            test_passed = False # Mark as failed if cleanup fails, as it affects future runs
        else:
            logger.info(f"Successfully deleted wedding {wedding_id} during cleanup.")
            # Verify deletion
            ...
            if read_after_delete_result.get("data"):
                logger.error(f"Wedding {wedding_id} still found after deletion verification.")
                test_passed = False
            else:
                logger.info(f"Wedding {wedding_id} successfully verified as deleted.")

    if test_passed:
        logger.info("--- Weddings CRUD Test Passed ---")
    else:
        logger.error("--- Weddings CRUD Test FAILED ---")
    return test_passed
...
async def test_wedding_members_crud():
    ...
    try:
        ...
        if wedding_insert_result.get("error") or not wedding_insert_result.get("data"):
            logger.error(f"Setup failed: Could not insert wedding for wedding_members test: {wedding_insert_result.get('error', 'No data returned')}")
            test_passed = False
            return False # Exit early if setup fails
        logger.info(f"Setup: Inserted wedding {wedding_id} for wedding_members test.")

        # 1. Insert Test
        logger.info(f"Attempting to insert wedding member: User {TEST_USER_ID} to Wedding {wedding_id}")
        insert_sql = """
        ...
        insert_result = await execute_supabase_sql(insert_sql, insert_params)
        if insert_result.get("error") or not insert_result.get("data"):
            logger.error(f"Insert wedding member failed: {insert_result.get('error', 'No data returned')}")
            test_passed = False
        else:
            ...
            if inserted_member.get("wedding_id") != wedding_id or inserted_member.get("user_id") != TEST_USER_ID:
                logger.error(f"Insert wedding member verification failed. Expected wedding_id {wedding_id}, user_id {TEST_USER_ID}. Got {inserted_member.get('wedding_id')}, {inserted_member.get('user_id')}")
                test_passed = False
            else:
                logger.info(f"Successfully inserted wedding member: {inserted_member}")

        # 2. Read Test
        if test_passed:
            logger.info(f"Attempting to read wedding member: User {TEST_USER_ID} from Wedding {wedding_id}")
            ...
            read_result = await execute_supabase_sql(read_sql, read_params)
            if read_result.get("error") or not read_result.get("data"):
                logger.error(f"Read wedding member failed: {read_result.get('error', 'No data returned')}")
                test_passed = False
            else:
                read_member = read_result["data"][0]
                if read_member.get("role") != member_role:
                    logger.error(f"Read wedding member verification failed. Expected role {member_role}, got {read_member.get('role')}")
                    test_passed = False
                else:
                    logger.info(f"Successfully read wedding member: {read_member}")

        # 3. Negative Test (Duplicate Insert)
        if test_passed:
            logger.info("Attempting duplicate insert for wedding member (expected to fail)...")
            ...
            if not duplicate_insert_result.get("error"):
                logger.error("Duplicate insert unexpectedly succeeded for wedding member.")
                test_passed = False
            else:
                logger.info(f"Duplicate insert failed as expected: {duplicate_insert_result['error']}")

    finally:
        # Cleanup: Delete the created wedding member
        logger.info(f"Cleanup: Attempting to delete wedding member (User {TEST_USER_ID} from Wedding {wedding_id})")
        ...
        delete_result = await execute_supabase_sql(delete_sql, delete_params)
        if delete_result.get("error") or not delete_result.get("data"):
            logger.error(f"Cleanup failed: Could not delete wedding member: {delete_result.get('error', 'No data returned')}")
            test_passed = False
        else:
            logger.info(f"Successfully deleted wedding member during cleanup.")
            # Verify deletion
            ...
            read_after_delete_result = await execute_supabase_sql(read_after_delete_sql, read_after_delete_params)
            if read_after_delete_result.get("data"):
                logger.error(f"Wedding member (User {TEST_USER_ID} in Wedding {wedding_id}) still found after deletion verification.")
                test_passed = False
            else:
                logger.info(f"Wedding member (User {TEST_USER_ID} in Wedding {wedding_id}) successfully verified as deleted.")
        ...
        logger.info(f"Cleanup: Deleted wedding {wedding_id}.")

    if test_passed:
        logger.info("--- Wedding Members CRUD Test Passed ---")
    else:
        logger.error("--- Wedding Members CRUD Test FAILED ---")
    return test_passed
...
async def test_tasks_crud():
    ...
    try:
        ...
        wedding_insert_result = await execute_supabase_sql(insert_wedding_sql, insert_wedding_params)
        if wedding_insert_result.get("error") or not wedding_insert_result.get("data"):
            logger.error(f"Setup failed: Could not insert wedding for tasks test: {wedding_insert_result.get('error', 'No data returned')}")
            test_passed = False
            return False # Exit early if setup fails
        logger.info(f"Setup: Inserted wedding {wedding_id} for tasks test.")

        # 1. Insert Test
        logger.info(f"Attempting to insert task: {task_title} for Wedding {wedding_id}")
        insert_sql = """
        ...
        insert_result = await execute_supabase_sql(insert_sql, insert_params)
        if insert_result.get("error") or not insert_result.get("data"):
            logger.error(f"Insert task failed: {insert_result.get('error', 'No data returned')}")
            test_passed = False
        else:
            inserted_task = insert_result["data"][0]
            if inserted_task.get("task_id") != task_id or inserted_task.get("wedding_id") != wedding_id:
                logger.error(f"Insert task verification failed. Expected ID {task_id}, wedding_id {wedding_id}. Got {inserted_task.get('task_id')}, {inserted_task.get('wedding_id')}")
                test_passed = False
            else:
                logger.info(f"Successfully inserted task: {inserted_task}")

        # 2. Read Test
        if test_passed:
            logger.info(f"Attempting to read task with ID: {task_id}")
            read_sql = "SELECT * FROM tasks WHERE task_id = :task_id;"
            read_params = {"task_id": task_id}
            read_result = await execute_supabase_sql(read_sql, read_params)
            if read_result.get("error") or not read_result.get("data"):
                logger.error(f"Read task failed: {read_result.get('error', 'No data returned')}")
                test_passed = False
            else:
                read_task = read_result["data"][0]
                if read_task.get("title") != task_title:
                    logger.error(f"Read task verification failed. Expected title {task_title}, got {read_task.get('title')}")
                    test_passed = False
                else:
                    logger.info(f"Successfully read task: {read_task}")
        ...
        if test_passed:
            ...
            update_result = await execute_supabase_sql(update_sql, update_params)
            if update_result.get("error") or not update_result.get("data"):
                logger.error(f"Update task failed: {update_result.get('error', 'No data returned')}")
                test_passed = False
            else:
                updated_task = update_result["data"][0]
                if updated_task.get("title") != updated_title or updated_task.get("status") != updated_status:
                    logger.error(f"Update task verification failed. Expected title {updated_title}, status {updated_status}. Got {updated_task.get('title')}, {updated_task.get('status')}")
                    test_passed = False
                else:
                    logger.info(f"Successfully updated task: {updated_task}")

        # 4. Negative Test (Invalid Foreign Key)
        if test_passed:
            logger.info("Attempting to insert task with non-existent wedding_id (expected to fail)...")
            ...
            if not invalid_insert_result.get("error"):
                logger.error("Invalid foreign key insert unexpectedly succeeded for tasks.")
                test_passed = False
            else:
                logger.info(f"Invalid foreign key insert failed as expected: {invalid_insert_result['error']}")

    finally:
        # Cleanup: Delete the created task
        logger.info(f"Cleanup: Attempting to delete task {task_id}")
        delete_sql = "DELETE FROM tasks WHERE task_id = :task_id RETURNING task_id;"
        delete_params = {"task_id": task_id}
        delete_result = await execute_supabase_sql(delete_sql, delete_params)
        if delete_result.get("error") or not delete_result.get("data"):
            logger.error(f"Cleanup failed: Could not delete task {task_id}: {delete_result.get('error', 'No data returned')}")
            test_passed = False
        else:
            logger.info(f"Successfully deleted task {task_id} during cleanup.")
            # Verify deletion
            ...
            read_after_delete_result = await execute_supabase_sql(read_after_delete_sql, read_after_delete_params)
            if read_after_delete_result.get("data"):
                logger.error(f"Task {task_id} still found after deletion verification.")
                test_passed = False
            else:
    ...
    if test_passed:
        logger.info("--- Tasks CRUD Test Passed ---")
    else:
        logger.error("--- Tasks CRUD Test FAILED ---")
    return test_passed

# @pytest.mark.asyncio
# async def run_all_db_tests():
#     logger.info("Starting all database CRUD tests...")
#     results = {
